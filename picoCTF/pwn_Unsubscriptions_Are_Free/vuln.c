...
void printMenu() {
  ...
  puts("==========================");
  puts("(S)ubscribe to my channel");//leak flag
  puts("(I)nquire about account deletion");//free user
  puts("(M)ake an Twixer account");//change username
  puts("(P)ay for premium membership");
  puts("(l)eave a message(with or without logging in)");//malloc msg
  puts("(e)xit");
}
