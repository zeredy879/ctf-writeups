    ...
    public static String decodeUtf8Array(byte[] bytes, int index, int size) {
        ...
        if ((index | size | ((bytes.length - index) - size)) >= 0) {
            ...
            while (offset < limit) {
                ...
                byte offset4 = bytes[offset];
                if (Utf8.DecodeUtil.isOneByte(offset4)) {
                    int resultPos3 = resultPos2 + 1;
                    Utf8.DecodeUtil.handleOneByte(offset4, resultArr, resultPos2);
                    while (offset3 < limit) {
                ...
                } else if (Utf8.DecodeUtil.isTwoBytes(offset4) != 0) {
                    if (offset3 < limit) {
                        Utf8.DecodeUtil.handleTwoBytes(offset4, bytes[offset3], resultArr, resultPos2);
                        offset = offset3 + 1;
                        resultPos2++;
                    } else {
                        throw new IllegalArgumentException("Invalid UTF-8");
                    }
                } else if (Utf8.DecodeUtil.isThreeBytes(offset4)) {
                    if (offset3 < limit - 1) {
                        int offset5 = offset3 + 1;
                        Utf8.DecodeUtil.handleThreeBytes(offset4, bytes[offset3], bytes[offset5], resultArr, resultPos2);
                        offset = offset5 + 1;
                        resultPos2++;
                    } else {
                        throw new IllegalArgumentException("Invalid UTF-8");
                    ...
                    Utf8.DecodeUtil.handleFourBytes(offset4, b3, bytes[offset6], bytes[offset7], resultArr, resultPos2);
                    offset = offset7 + 1;
